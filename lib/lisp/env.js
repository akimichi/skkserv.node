...
const Self = {
  ...
  prelude: (environment) => {
    const x = Exp.variable("x"),
      ...
      _ = Exp.variable("_");
    const compile = (inputString) => {
      const Parser = require('./parser.js');
      return List.match(Parser.parse(Parser.expression)(List.fromString(inputString)),{
        empty: (_) => {
          throw new Error(`'Parse Error: ${inputString}`)
        },
        cons: (head, tail) => {
    ...
    const builtinFunctions = List.fromVector([
      ...
      // Pair.cons("succ", compile('(lambda (n) (succ n))')),
