...
const Self = {
  ...
  flatMap: (parser) => {
    return (f) => {
      return (input) => {
        ...
        // return List.join(Pair.match(List.head(Self.parse(parser)(input)),{
        //   cons: (v, inp_) => {
        //     return Self.parse(f(v))(inp_);
        //   },
        //   empty: () => {
        //     return List.empty();
        //   }
  ...
  many: (parser) => {
    return Self.plus(Self.some(parser))(Self.pure(List.empty()));
  },
  ...
  numeric: () => {
    ...
    return Self.fmap(toExp)(Self.token(Self.plus(Self.float)(Self.int)));
