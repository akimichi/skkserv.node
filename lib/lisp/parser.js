...
const Self = {
  ...
  //              | list 
  expression: (input) => {
    return (Self.alt(Self.atom,Self.alt(Self.variable,Self.alt(Self.ifExpr,Self.alt(Self.list,Self.app)))))(input);
    // return (Self.plus(Self.atom)(Self.plus(Self.variable)(Self.plus(Self.app)(Self.plus(Self.ifExpr)(Self.list)))))(input);
  ...
  // if = "(" if s_expression s_expression  s_expression  ")"
  ifExpr: (input) => {
    const ifChars = Self.token(Self.chars(List.fromString("if"))),
      open = Self.char("("),
      close = Self.char(")");
    return Self.flatMap(open)(_ => {
      return Self.flatMap(ifChars)(_ => {
        return Self.flatMap(Self.token(Self.expression))(predicate => {
          return Self.flatMap(Self.token(Self.expression))(consequent => {
            return Self.flatMap(Self.token(Self.expression))(alternative => {
              return Self.flatMap(close)(_ => {
