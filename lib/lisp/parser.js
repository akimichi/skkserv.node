...
const Self = {
  ...
  //              | list 
  expression: (input) => {
    return Self.plus(Self.plus(Self.atom)(Self.boolean))(Self.string)(input);
  },
  ...
  list: (input) => {
    const open = Self.char("("),
      close = Self.char(")");
    return Self.flatMap(open)(_ => {
      return Self.flatMap(Self.many(Self.expression))(items=> {
        return Self.flatMap(close)(_ => {
  ...
  // variable: identifier
  variable: (input) => {
    return Self.identifier(List.unit("lambda"))
  },
  // lambda
  /*
   * (lambda (仮引数) (演算式))
