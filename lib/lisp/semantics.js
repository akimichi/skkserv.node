...
const evaluate = (anExp, environment) => {
  return exp.match(anExp,{
    ...
    if: (predicate, trueExp, falseExp) => {          
      return ID.flatMap(evaluate(predicate, environment))((judge) => {
        if(judge === true) {
          return ID.flatMap(evaluate(trueExp, environment))((value) => {
            return ID.unit(value); 
          });
        } else {
          return ID.flatMap(evaluate(falseExp, environment))((value) => {
            return ID.unit(value); 
          });
        }
      });
